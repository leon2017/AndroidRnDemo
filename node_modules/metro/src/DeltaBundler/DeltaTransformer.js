...
class DeltaTransformer extends EventEmitter {
  ...
  bundleOptions)
  {
    ...
    // the correct module).
    this._getModuleId = this._bundleOptions.isolateModuleIDs ? (bundleOptions.createModuleIdFactory || createModuleIdFactory)() : globalCreateModuleId;this._deltaCalculator.on('change', this._onFileChange);}static create(bundler, options, bundleOptions) {return _asyncToGenerator(function* () {const resolver = yield bundler.getResolver();const deltaCalculator = new DeltaCalculator(bundler, resolver.getDependencyGraph(), bundleOptions);return new DeltaTransformer(bundler, resolver, deltaCalculator, options, bundleOptions);})();} /**
      ...
      var _ref4 = yield _this5._deltaCalculator.getDelta();const modified = _ref4.modified,deleted = _ref4.deleted,reset = _ref4.reset;const transformerOptions = yield _this5._deltaCalculator.getTransformerOptions();const dependencyEdges = _this5._deltaCalculator.getDependencyEdges();try {// Return the source code that gets prepended to all the modules. This
        ...
        _this5._deltaCalculator.reset();throw e;}})();}_getDeps(path, edges, deps) {if (deps.has(path)) {return deps;}const edge = edges.get(path);if (!edge) {return deps;}deps.add(path);for (const _ref5 of edge.dependencies) {var _ref6 = _slicedToArray(_ref5, 2);const dependencyPath = _ref6[1];this._getDeps(dependencyPath, edges, deps);}return deps;}_getPrepend(transformOptions, dependencyEdges) {var _this6 = this;return _asyncToGenerator(function* () {// Get all the polyfills from the relevant option params (the
      // `getPolyfills()` method and the `polyfillModuleNames` variable).
      const polyfillModuleNames = _this6._getPolyfills({ platform: _this6._bundleOptions.platform }).concat(_this6._polyfillModuleNames); // The module system dependencies are scripts that need to be included at
      // the very beginning of the bundle (before any polyfill).
      const moduleSystemDeps = _this6._resolver.getModuleSystemDependencies({ dev: _this6._bundleOptions.dev });const modules = moduleSystemDeps.concat(polyfillModuleNames.map(function (polyfillModuleName, idx) {return _this6._resolver.getDependencyGraph().createPolyfill({ file: polyfillModuleName, id: polyfillModuleName, dependencies: [] });}));return yield _this6._transformModules(modules, transformOptions, dependencyEdges);})();}_getAppend(dependencyEdges) {var _this7 = this;return _asyncToGenerator(function* () {const dependencyGraph = _this7._resolver.getDependencyGraph(); // Get the absolute path of the entry file, in order to be able to get the
      // actual correspondant module (and its moduleId) to be able to add the
      // correct require(); call at the very end of the bundle.
      const entryPointModulePath = dependencyGraph.getAbsolutePath(_this7._bundleOptions.entryFile); // First, get the modules correspondant to all the module names defined in
